...
const updateUserByEmail = async (email, userData) => {
    const { first_name, last_name, birth_date, gender, phone, new_email, profile_pic, password } = userData;
    const res = await pool.query(
        ...
        [first_name, last_name, birth_date, gender, phone, new_email, password, profile_pic, email]
    );
    return res.rows[0];
};
